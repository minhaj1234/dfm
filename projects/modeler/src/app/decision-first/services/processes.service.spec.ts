...
describe('ProcessesService', () => {
  ...
  beforeEach(async(() => {
    ...
    TestBed.configureTestingModule({
      ...
      providers: [ProcessesService, { provide: ExternalService, useValue: fakeExternalService }],
    });
  ...
  it('converts a single process from the server into a Process; this makes sure links are sanitized', async(() => {
    service.getSingleEdit('process1000').subscribe((processes) => {
      expect(processes).toEqual(jasmine.any(Process));
    });

    fakeExternalService._next(processData._embedded.processes[0]);
  }));
  ...
  describe('getSomeMinimalWithSearch', () => {
    function getData() {
      return {
        _embedded: {
          processes: processData._embedded.processes,
        },
        _links: {
          next: {
            href: 'http://',
    ...
    it('converts an array the array into Process[]', () => {
      service.getSomeMinimalWithSearch().subscribe((response) => {
        expect(response.results.every((graphable) => graphable instanceof Process)).toBe(true);
      });

      fakeExternalService._next(getData());
    });

    it('does not fail if there are no processes', () => {
      let results;
      ...
      const data = getData();
      delete data._embedded.processes;
